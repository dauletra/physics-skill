...
/* Подзаголовок раздела документа (блок `heading`); внутри заданий
   заголовков нет — иерархию там рисуют номер и буквы подзаданий. */
...
/* Секция «Ответы» в конце документа (build_answers_section, document.py):
   отбита линией от тел заданий; по строке на вопрос — номер задания
   (и буква подзадания) слева, компактный ответ справа. Ответ-график (plot)
   рисуется тем же svg.visual-svg — правило ширины ниже повторяет
   .task-block, масштаб SVG единый по всему документу. */
...
/* flex-start, не baseline: у ответа-графика (plot) первая базовая линия —
   подпись легенды под SVG, и номер уезжал бы под график. */
...
.answers-body svg.visual-svg { display: block; width: 100%; max-width: 330px; height: auto; }
.answers-explanation { font-style: italic; color: #333; }
/* Блоки задания идут друг под другом на всю ширину колонки; единственный
   способ поставить блоки рядом — layout-контейнер `row` (см. document-schema.md).
...
/* Layout-контейнер `row` — дети стоят в одну строку и делят её поровну
   (см. document-schema.md). Грид с равными колонками сам учитывает gap —
