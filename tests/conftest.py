...
MINIMAL_VISUALS = {
    ...
    # Мелкая сетка внутри деления (`grid: "fine"`): диапазон, где крайние
    # подписанные деления его не покрывают (250…650 подписывается с 300) —
    # промежуточные линии обязаны дойти до самого края поля.
    "graph-fine-grid": {
        "type": "graph", "x_label": "T, К", "y_label": "p, кПа",
        "x_range": [250, 650], "y_range": [0, 250], "grid": "fine",
        "series": [{"points": [[300, 100], [600, 200]]}],
    },
    "instrument-ruler": {
        "type": "instrument", "kind": "ruler", "unit": "см",
