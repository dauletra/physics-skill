"""Уровень 2: чистые компоненты — только отрисовка, никакого знания об
...
`from_dict()` (сырой JSON-элемент `Task.items`) вызывает `esc()` на авторском
тексте; классы вопросов (уровень 3), которые строят компонент программно,
