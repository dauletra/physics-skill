...
**Спроектирована, но не реализована новая архитектура (v2)** — три уровня
абстракции: (1) JSON конкретного вида вопроса, содержащий и контент, и ответ
вместе, своей формы для каждого вида; (2) чистые компоненты `text`/`table`/
`graph`/`list` — только отрисовка, никакого знания об ответе/правильности;
(3) по классу на педагогический вид (`ChoiceQuestion`, `MatchingQuestion` и
т.п.), который хранит данные и сам решает, что передать компонентам в
ученик/учитель-режиме. `Task { items: (Component | Question)[] }` — один
плоский разнородный список вместо флага `answerable` или отдельной сущности
для общего контекста: буква/ответ определяются тем, компонент это или
вопрос. Общий Layout DSL на одном уровне (`Task.layout` партиционирует
`items`). Полный дизайн, каталог видов вопросов и рабочие примеры — в
`worksheet-builder/references/task-schema-v2-draft.md`. Пока не реализовано
— `scripts/`/`task-schema.md`/примеры продолжают работать по старой модели
(`item_style`, `settings`), не анкеруй новые архитектурные решения на неё,
она будет заменена целиком при реализации v2.
